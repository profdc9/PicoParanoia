...
int fileenc_base64_writedata(int c, void *v)
{
  ...
  return 0;   // PicoParanoia fix: the ParanoiaBox original omits this return.
              // Same class of bug as fileblockencode_writedata (see fileop.cpp):
              // the caller, base64_encode, ignores the value, but a missing
              // return in a non-void function is UB that HardFaults on this
              // toolchain/target. Caught by -Werror=return-type.
}
