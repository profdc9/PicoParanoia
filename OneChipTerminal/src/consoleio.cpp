...
//
// Input: the PS/2 keyboard FIFO only (this project has no USB and no debug
// console sharing uart0 -- uart0 is the terminal's own data link, wired up
// separately in main.cpp).
...
// 80x25: this project is specifically an 80-column VT100 terminal (unlike
// PicoParanoia's 40-column console), so PICO_NTSC_MODE_80 (8px cells, single
// width) rather than the 40-col double-width mode.
#define CON_ROWS 25
#define CON_COLS 80
...
{
    pico_ntsc_init(PICO_NTSC_MODE_80);      // 80-col, unscii-8; sets sysclk 126 MHz
...
{
    // ~60.05 Hz frames; no frame counter exposed yet, so approximate with
    // sleep. Unused by OneChipTerminal.ino today, kept for API parity.
