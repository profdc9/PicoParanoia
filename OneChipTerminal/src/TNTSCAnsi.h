...
class TNTSCAnsi_class {    
  public:
    ...
    void end();
    // Direct pokes for callers outside the normal ANSI/escape-sequence path
    // (e.g. a config menu overlay) that still need the framebuffer hook to
    // fire -- see TNTSCAnsi.cpp for why a raw cur.data write isn't enough.
    void poke_string(int row, int col, const char *s);
    void poke_region(int row, int col, const unsigned char *src, int n);
//  private:
