...
// Optional per-cell change hook. A host project can define
// TNTSCANSI_CELL_CHANGED(cur, index, ch) (via the auto-included tntscansi_hook.h)
// to be notified with the linear buffer index whenever a cell changes value, so
// it can blit just that cell. Default is a no-op, which keeps this file a
// standalone VT100 text-buffer emulator with unchanged behavior.
#if defined(__has_include)
#  if __has_include("tntscansi_hook.h")
#    include "tntscansi_hook.h"
#  endif
#endif
#ifndef TNTSCANSI_CELL_CHANGED
...
#define VS_PUT(cur, ptr, val) do {                                  \
    ...
    screenchartype vv_ = (screenchartype)(val);                     \
    *vp_ = vv_;                                                      \
    TNTSCANSI_CELL_CHANGED((cur), (int)(vp_ - (cur).data), vv_);     \
} while (0)
...
// Direct, non-ANSI writes into the virtscreen -- for callers (OneChipTerminal's
// config menu) that poke text onto the screen outside the normal escape-sequence
// path. Still routed through VS_PUT so every character reaches the framebuffer
// hook, unlike a raw memcpy/memset into cur.data.
void TNTSCAnsi_class::poke_string(int row, int col, const char *s)
{
  screenchartype *ch = loc_in_virtscreen(cur, row, col);
  while (*s) VS_PUT(cur, ch++, (unsigned char)*s++);
}

void TNTSCAnsi_class::poke_region(int row, int col, const unsigned char *src, int n)
{
  screenchartype *ch = loc_in_virtscreen(cur, row, col);
  while (n-- > 0) VS_PUT(cur, ch++, *src++);
}
...
void TNTSCAnsi_class::delete_chars_in_line(int char_move)
{
   ...
   if ((cur.xpos + char_move) < cur.columns)
   {
     ...
     while (next_move<end_move) { VS_PUT(cur, start_move, *next_move); start_move++; next_move++; }
   }
   while (start_move<end_move) { VS_PUT(cur, start_move, clr_with); start_move++; }
}
...
void TNTSCAnsi_class::scroll_virt_up_at_cursor(int dir)
{
       ...
       (cur.ypos < cur.bottom_scroll))
   {
     ...
     if (dir)
       {
	 for (unsigned int k=0;k<real_scroll_length;k++) VS_PUT(cur, beginScr+k, nextLine[k]);
         while (lastLine < endofScr) { VS_PUT(cur, lastLine, clr_with); lastLine++; }
       } else
	 {
	   for (int k=(int)real_scroll_length-1;k>=0;k--) VS_PUT(cur, nextLine+k, beginScr[k]);
           while (beginScr < nextLine) { VS_PUT(cur, beginScr, clr_with); beginScr++; }
	 }
   }
}
...
{
#ifdef VTATTRIB
  VS_PUT(cur, loc_in_virtscreen(cur,cur.ypos,cur.xpos), ch | (cur.attrib << 8));
#else
  VS_PUT(cur, loc_in_virtscreen(cur,cur.ypos,cur.xpos), ch | ((cur.attrib & 0x08) << 4));
#endif
}
