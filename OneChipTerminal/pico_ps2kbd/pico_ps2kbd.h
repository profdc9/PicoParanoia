...
// Live Alt-key state (held down or not), for callers that need a modifier
// as a continuous state rather than folded into a decoded byte (e.g. "hold
// Alt to enter a config menu"). Not used by every caller; tracked
// unconditionally since it costs nothing idle.
uint8_t pico_ps2kbd_altkey(void);
